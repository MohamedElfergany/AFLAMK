...
import { getSeriesDetails } from "../../redux/Slice/AllSeriesSlice";
import { useParams, Link } from "react-router-dom";
import { FaSpinner } from "react-icons/fa";

const SeriesDetails = () => {
  const { detailsSeries, loading, error } = useSelector(
    (state) => state.myAllSeries
  );
  const dispatch = useDispatch();
  const { seriesId } = useParams();

  useEffect(() => {
    console.log("seriesId from URL:", seriesId);
    if (seriesId) {
  ...
  if (error && typeof error === "object" && error.status_message) {
    return <p>{error.status_message}</p>;
  }
          ...
          <>
            <div className="flex flex-col md:flex-row items-center w-full max-w-5xl">
              <div className="p-4 w-full md:w-1/3 flex justify-center">
                <img
                  src={`https://image.tmdb.org/t/p/w500/${detailsSeries.poster_path}`}
                  alt={detailsSeries.title}
              ...
              <div className="text-left p-4 w-full md:w-2/3">
                <h1 className="text-2xl md:text-4xl font-bold mb-4 text-center md:text-left">
                  {detailsSeries.title} ({detailsSeries.release_date ?? 2024})
                </h1>
